...
<div class="form-card">
    <form action="{{ route('admin.teachers.update', $teacher) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="number" class="form-control @error('experience') is-invalid @enderror" id="experience" name="experience" value="{{ old('experience', $teacher->experience) }}" min="0" required>
            @error('experience')
            <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            @if($teacher->photo)
                <div>
                    <img src="{{ asset('storage/' . $teacher->photo) }}" class="current-image" alt="{{ $teacher->name }}">
                    <p style="color:#718096;font-size:0.875rem;margin-top:0.5rem;">Foto saat ini</p>
                </div>
            @endif
            <input type="file" class="form-control @error('photo') is-invalid @enderror" id="photo" name="photo" accept="image/*" onchange="previewImage(this)" style="margin-top:1rem;">
        ...
        <div class="btn-group">
            <button type="submit" class="btn-submit">💾 Update Guru</button>
